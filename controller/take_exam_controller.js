...
router.get('/', isLoggedInAsStudent, (req, res)=> {

    res.render('exams/take_exam_code_entry', {title: "Enter Exam Code", username: req.user.username});

});

router.post('/exam', isLoggedInAsStudent, (req, res)=> {
	var exam_code = req.body.exam_code;
	var username = req.body.username;
    console.log(exam_code);
    console.log(username);
    Exam.getByExamCode(exam_code, (err,doc1) =>
    {
        if(err)
            res.send("Some error occured");
        else if(doc1)
        {
            Student.getBycourseid(username, doc1.course_code, (err,doc)=>
            {
                if(err)
                    res.send("Some error occured");
                else if(doc)
                {
                    Exam.checkResponse(username, exam_code, (err, doc)=> 
                    {
                        if(err)
                            res.send("Some error occured");
                        else if(doc)
...
router.get('/list', isLoggedInAsStudent, (req, res)=> {
...
router.post('/submit', isLoggedInAsStudent, (req, res)=> {
    ...
    Exam.addResponses(username, exam_code, response, (err,docs)=>{
        if(err)
        res.send("some error occured");
        else
...
router.get('/performance', isLoggedInAsStudent, (req, res)=> {

    res.render('exams/view_performance_code_entry', {title: "Enter Exam Code", username: req.user.username});

});

router.post('/performance_view', isLoggedInAsStudent, (req, res)=> {
    ...
    Exam.getByExamCode(exam_code, (err,docs)=>{
        ...
        else
        {
           var exam_obj = docs.question_list;
            ...
            Exam.getResponseByExamCode(exam_code, username, (err, docs)=>{
                if(err)
                    res.send("some error occured");
                else
                {
